...
**Vega BoN-6 (89.4)** ([[sources/vega.md]]) is the current wiki NAVSIM-v2 SOTA — but **no direct head-to-head** with DriveDreamer-Policy (88.7), DreamerAD (87.7), or Senna-2 (86.6). The +0.7 margin over DDP is plausible but unverified. Vega uses BoN-6 (best of 6 samples), which is a different inference regime from DDP's single-sample 88.7.
