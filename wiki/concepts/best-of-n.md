...
## BoN Scores Are Leaking Into Single-Sample Tables

This page exists partly to keep oracle Best-of-N numbers separate from deployable single-sample ones. [[sources/wcog-vla.md]] is the wiki's first documented case of that separation failing **inside another paper's comparison table**.

Its NAVSIM-v1 table lists **AutoVLA-3B at 92.1 PDMS** alongside ReCogDrive-2B (90.8) and LatentVLA-3B (92.4), with no marker distinguishing it. But:

| AutoVLA configuration | PDMS | Source |
| --- | ---: | --- |
| One-shot SFT | 80.54 | [[sources/autovla.md]] Table 1 |
| **Post-RFT (single-sample)** | **89.11** | [[sources/autovla.md]] Table 1 |
| **Best-of-N (oracle, 6 candidates)** | **92.12** | [[sources/autovla.md]] Table 1 |

92.1 is the **oracle** row. WCog-VLA's stated claim — that it "surpasses the RL-refined VLM-based methods, ReCogDrive and AutoVLA, by at least 0.8 PDMS" — is therefore measured against a configuration that needs the NAVSIM simulator at inference to pick the best of six samples. Against AutoVLA's deployable score the margin is **3.8**, not 0.8.

**Why this matters beyond one row.** BoN scores sit systematically 3–7 points above their single-sample counterparts in this wiki (AutoVLA +3.0, DriveVLA-W0 +4.6, ExploreVLA +3.3, Curious-VLA +4.5, NoRD +6.8). Once one paper carries a BoN figure into a single-sample table, later papers copying that baseline row inherit it — the same copying mechanism documented for [evaluator drift](navsim-benchmark.md#three-protocols). The practical rule is the same one that page reaches: **check the source paper's own table before treating a baseline row as commensurable**, and for VLA methods specifically, check which of the three configurations (SFT / post-RFT / BoN) it came from.
