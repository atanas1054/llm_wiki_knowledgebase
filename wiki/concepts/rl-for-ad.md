...
## Plan-R1: VD-GRPO for Safety-Critical Rewards

**Plan-R1** ([[sources/plan-r1.md]]) gives a sharper optimizer-level diagnosis of GRPO in driving. Its reward uses multiplicative safety gates for collision and drivable-area compliance, then soft terms for TTC, comfort, speed compliance, and progress. Standard GRPO improves overall nuPlan score but lowers collision avoidance, because per-group standard-deviation normalization implicitly weights each group by `1 / sigma_R`.

In Plan-R1's analysis, unsafe groups have higher reward variance, so standard GRPO downweights the rare safety-critical cases that should dominate training. **VD-GRPO** replaces `(R - mean) / std` with `(R - mean) / c`, preserving absolute reward scale across groups. In Table 2, VD-GRPO improves over GRPO by +2.58 NR-CLS, +1.69 R-CLS, +3.45 collision, and +0.39 drivable compliance.

Plan-R1 also differs from non-reactive NAVSIM GRPO: during RL, a trainable ego policy interacts with a frozen pretrained motion-token predictor used as a reactive world model for surrounding agents. This makes it an important bridge between simulator-assisted RL and world-model-based RL.

## PlannerRFT: PPO-Guided Exploration plus GRPO

[[sources/plannerrft.md]] separates the problem of **where to explore** from **how to update the trajectory distribution**. A compact Exploration Policy uses PPO to predict scene-conditioned Beta distributions over lateral and longitudinal guidance. The DiT generator uses GRPO to learn from the resulting trajectory groups.

The ablation demonstrates that maximizing diversity is not sufficient: uniform guidance reaches 39.78% diversity but lowers Test14-hard reactive score below the imitation baseline, while adaptive guidance reaches 72.21 with 25.34% diversity and a higher mean group reward. Exploration must be both multimodal and compatible with the scene.

PlannerRFT also introduces survival reward for hard groups. Terminal reward makes all candidates equivalent once they fail; accumulating valid non-terminal prefixes retains a relative signal for delaying collision or off-road failure. The strongest setting uses a four-second horizon, group size eight, and a balanced below-90-score training subset rather than failure-only cases.

## PaIR-Drive: RL as a Reusable Parallel Refiner

[[sources/pair-drive.md]] rejects fine-tuning the IL policy entirely. A separate RL branch learns intention-conditioned residual trajectories around human references with simulator-scored GRPO. At deployment, it is centered on an IL proposal and retained as a proposal module; an RWM ranks the trajectory tree.

This changes the IL/RL tradeoff. Direct gradient conflict and catastrophic drift are structurally avoided because the networks are separate, and one RL branch improves both TransFuser and DiffusionDrive without RL retraining. In exchange, the system introduces training-to-inference reference shift, extra candidate-selection cost, and dependence on an under-specified reward model.

The single-plan comparisons are the clean evidence: TransFuser improves 84.0→89.7 PDMS and 79.7→86.6 EPDMS; DiffusionDrive improves 88.1→91.2 and 84.3→87.9. Peak 94.0/89.6 results use Best-of-6 and should be categorized separately.

## DIAL: Preference Contrast through Intent-Balanced Groups

[[sources/dial.md]] argues that GRPO stagnation can originate before advantage computation. If all candidates occupy one maneuver basin, RFS varies little and group-relative updates have no semantic preference contrast. DIAL first expands a flow policy with eight intent-conditioned CFG modes, then constructs every 16-sample group as two trajectories per intent.

At a fixed group budget, multi-intent GRPO reaches held-out RFS 8.211; random single-intent groups peak at 7.992 and predicted/GT/top-rater variants perform worse. The method retains ordinary group mean/std normalization and adds a reference-path penalty, so its primary contribution is sampling support rather than an optimizer correction.

DIAL also modifies training RFS to reduce reward hacking: it replaces hard max-over-raters with label-softmax aggregation and scores 1–5 s rather than only 3 s and 5 s. The strongest held-out peak uses temperature 0.3, while temperature 1.0 gives the strongest full-pool score, showing that reward/checkpoint choice remains consequential.

## DisCO: A Binary-Reward Objective Alternative

[[sources/disco.md]] is not an AD paper, but its GRPO analysis matters for binary safety gates. It shows that normalized GRPO weights a mixed group's discriminative objective by $\sqrt{p(1-p)}$ and Dr. GRPO retains $p(1-p)$. A direct positive-versus-negative objective can remove this weighting, avoid clipping, focus on high-scoring hard negatives, and constrain policy drift with a KL threshold.

Direct driving transfer is not established. PDMS, EPDMS, RFS, comfort, and progress are continuous or composite. A DisCO-style planner would need pairwise/ordinal ranking, explicit handling for all-failed groups, and validation that hard-negative emphasis does not exploit simulator artifacts. PlannerRFT survival rewards, ELF-VLA recovery generation, and DIAL intent groups address cases that binary DisCO alone cannot learn from.

## DAPO: Transferable Systems Lessons and Safety Caveats

[[sources/dapo.md]] contributes four operational lessons: asymmetric upper clipping can preserve rare exploration actions; batches should track how many groups have usable reward contrast; loss reduction changes length/action-horizon weighting; and terminal truncation should not automatically be labeled failure.

Driving transfer requires different choices. Dynamic sampling discards all-failed groups, which may remove collision-heavy or off-road scenes that most need improvement. Clip-Higher increases exploration and therefore needs feasibility constraints. Token-level loss maps imperfectly to continuous trajectories because giving longer horizons more aggregate weight can distort temporal credit. Soft truncation rewards are relevant when a rollout times out, but must distinguish computational cutoff from genuine route failure.

DAPO removes reference KL entirely. That may be acceptable for verifiable math but is risky for driving policies whose imitation prior supplies broad safety and comfort behavior.
...
The optimizer correction comes from [[sources/understanding-r1-zero-like-training.md]], which shows in math reasoning that standard GRPO introduces response-level length bias and question-level difficulty bias. In that source, Dr. GRPO removes response-length normalization and per-question reward-std normalization. NoRD's contribution is to show the same std-normalization problem becomes practical in driving: high-variance intermediate scenes are exactly where a weak policy needs gradient signal, but standard GRPO attenuates them.
...
## SimWAM: Flow-GRPO SDE Exploration with LoRA-Only Updates

[[sources/simwam.md]] applies RL to a continuous flow-matching action expert and contributes three practical choices, each with a direct ablation.

**ODE → SDE for exploration.** A deterministic rectified-flow ODE produces one trajectory and has no tractable transition density, so it supports neither exploration nor policy gradients. Following Flow-GRPO, SimWAM substitutes a marginal-preserving SDE whose Euler-Maruyama steps are isotropic Gaussians with tractable log-likelihoods. The comparison against naive perturbation is informative:
...
Random perturbation explores harder and produces the highest Ego Progress in the paper (88.0, above ReCogDrive's 87.3), but costs 0.7 NC and 1.4 TTC through less structured maneuvers. This is the same safety-versus-progress signature that WorldRFT-style random noise shows elsewhere in this page.

**Tension with the DreamerAD framing.** The DreamerAD sampling-comparison table earlier on this page characterizes Flow-GRPO as suffering "training/inference mismatch → jagged trajectories." SimWAM's evidence does not support that as a general verdict: within its own setup the SDE is the *better* of the two stochastic samplers, improving exactly the safety metrics that jaggedness would degrade. The two papers differ in policy class (DreamerAD ranks a fixed 8192→256 vocabulary; SimWAM perturbs a continuous flow), so both can be right — vocabulary ranking may still be smoother, but "Flow-GRPO produces jagged trajectories" should be read as DreamerAD's argument for its own design rather than an established result.

**LoRA-only updates preserve the distilled prior.** Only rank-32 adapters ($\alpha=16$) on the action expert's attention projections are trained; the video expert and the rest of the action expert stay frozen. This is a lighter-touch answer to the reward-hacking problem that DriveFine identified for weakly-coupled diffusion planners than FLARE's BC regularization or DiffusionDriveV2's IL loss — constrain the *parameters* that can move rather than the distribution they can move to. SimWAM reports no EPDMS, so whether it avoids the PDMS-up/EPDMS-down signature cannot be checked.

**Hard-subset scene selection.** RL runs only on navtrain scenes where the imitation policy scores below 90 PDMS. Figure 3 shows this consistently beats training on all of navtrain: easy scenes are already solved and dilute the signal. This independently replicates [[sources/plannerrft.md]]'s below-90 subset finding, and sits between two poles already in this page — Curious-VLA's ADAS selects for *reward variance* and explicitly shows that difficulty-based selection can collapse training (35.2 PDMS), while SimWAM selects for *low absolute score*. The reconciliation is that SimWAM's threshold is mild (below 90 on a ~90-mean policy is a large fraction of scenes) rather than a failure-only filter, matching PlannerRFT's conclusion that balanced below-90 subsets beat failure-only ones.

Both RL curves decline slightly past 15k steps, so the reported 91.5 depends on early stopping selected against the evaluation benchmark.

**RL contribution**: 90.3 → 91.5 PDMS (+1.2), concentrated in EP (83.9 → 86.4) and DAC, with small NC and TTC regressions — smaller than ReCogDrive's +2.8, WAM-Flow's +3.6, or FLARE's +4.5, but applied on top of an already-strong 90.3 imitation policy.

## SGDrive: A Controlled Read on What RL Adds

[[sources/sgdrive.md]] contributes no new RL algorithm — it deliberately reuses **ReCogDrive's exact RL configuration** — which is precisely what makes it useful here. Because the optimizer, reward, and simulator are held fixed while the representation changes, the SFT→RFT deltas are comparable across the two papers in a way that most entries on this page are not:

| Model | SFT PDMS | RFT PDMS | Δ from RL |
| --- | ---: | ---: | ---: |
| ReCogDrive-2B | 86.5 | 90.8 | +4.3 |
| ReCogDrive-8B | 86.8 | 90.5 | +3.7 |
| SGDrive-2B | 87.4 | 91.1 | +3.7 |

Two observations. **A better SFT policy does not shrink the RL gain here** — SGDrive starts 0.6 above ReCogDrive-8B and gains the same +3.7, ending 0.6 ahead. The structured-world-knowledge representation and the RL stage are additive rather than substitutive, which is not guaranteed (Curious-VLA's Narrow Policy analysis elsewhere on this page describes exactly the opposite failure, where a stronger-fitting SFT stage starves RL of advantage signal). And **scale does not help RL either**: ReCogDrive-2B gains more from RL than the 8B version and ends higher.

The sub-metric signature is worth noting. RL moves SGDrive's EP from 81.2 to 85.8 (+4.6) and DAC from 95.1 to 97.8 (+2.7) while NC stays pinned at 98.6 — the same progress-and-compliance pattern SimWAM reports for its Flow-GRPO stage. Across both papers, PDMS-reward RL on an already-safe policy buys efficiency, not additional collision avoidance, because the NC term is already near ceiling. SGDrive's own EP after RL (85.8) still trails ReCogDrive-2B's (87.3), so its advantage is elsewhere.

The paper flags as future work that its RL is generic and not tailored to the hierarchical world-knowledge forecast — no reward touches the occupancy, agent, or goal predictions.
