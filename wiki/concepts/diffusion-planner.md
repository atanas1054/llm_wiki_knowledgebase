...
related: [sources/wcog-vla.md, sources/geowam.md, sources/wa-jepa.md, sources/drivelaw.md, sources/recogdrive.md, sources/wam-flow.md, sources/uniugp.md, sources/reflectdrive.md, sources/reasoning-vla.md, sources/orion.md, sources/linkvla.md, sources/drivefine.md, sources/autovla.md, sources/alpamayo-r1.md, sources/diffusiondrive.md, sources/diffusiondrive-v2.md, sources/drivevla-w0.md, sources/percept-wam.md, sources/wam-diff.md, sources/spanvla.md, sources/onedrive.md, sources/had.md, sources/feaxdrive.md, sources/clear.md, sources/plannerrft.md, sources/dial.md, concepts/rl-for-ad.md, concepts/vlm-domain-adaptation.md, concepts/discrete-flow-matching.md, concepts/world-model-for-ad.md, concepts/inference-time-safety.md, concepts/adaptive-routing.md, concepts/nuplan-benchmark.md, concepts/intent-conditioned-planning.md]
created: 2026-04-05
updated: 2026-09-04
confidence: high
---
...
## Single-Step Drift as a Diffusion Alternative

CLEAR ([[sources/clear.md]]) is relevant to this page because it explicitly targets diffusion's latency bottleneck. Instead of running an iterative denoising chain, CLEAR learns a single conditional drift in a VAE trajectory-latent space. A frozen PCA projection fitted on expert demonstrations decodes the latent output into smooth physical trajectories.

The trade-off is controlled by `alpha`: low `alpha` spreads candidates over multiple feasible positives, while high `alpha` collapses toward the expert ground truth. This gives CLEAR a diffusion-like diversity/precision knob without a diffusion-time loop. The risk is that a one-step drift may depend heavily on the quality of the VAE/PCA latent manifold and on the learned scheduler choosing a good `(alpha, N)` scheme.
...
## PlannerRFT: Learning the Exploration Distribution

[[sources/plannerrft.md]] argues that the denoising MDP is useful only when grouped samples contain meaningful alternatives. Vanilla Gaussian starts collapse to nearly the same trajectory, while fixed anchors include context-conflicting maneuvers. PlannerRFT learns Beta distributions over lateral and longitudinal classifier-guidance scales conditioned on the scene and a frozen reference trajectory.

The training decomposition is important:

- PPO trains the guidance policy from long-horizon closed-loop returns.
- GRPO shifts the five-step DDIM transition distribution toward higher-reward trajectories.
- A survival reward distinguishes early and late failures when terminal rewards would make the whole group zero.
- A behavior-cloning term and fixed imitation reference constrain policy drift.

The guidance machinery is removed at inference. The reported deployed path uses five DDIM steps at 34.27 ms, compared with 75.48 ms when reference-guided denoising remains enabled. This makes PlannerRFT a training-time exploration method, not an inference-time candidate search or safety layer.

## DIAL: Intent-CFG for Continuous Flow Support

[[sources/dial.md]] applies the same mode-collapse diagnosis to a flow-matching action head trained from one trajectory per scene. Instead of fixed anchors or learned lateral/longitudinal offsets, it conditions the continuous generator on eight discrete maneuver intents and trains conditional plus unconditional flow fields through classifier-free dropout.

This makes intent a support-control variable. Ordinary SFT sampling from four controlled baselines remains below logged-trajectory RFS even with 128 samples, whereas intent-conditioned proposals cross the log around eight samples and eight-intent pooling reaches RFS 9.14 at 128. Multi-intent GRPO then preserves those modes during reward optimization.

DIAL differs from PlannerRFT in deployment: PlannerRFT removes its exploration policy and reference branch; DIAL retains intent prediction and conditional generation. The paper does not report the latency cost of conditional/unconditional CFG passes.
...
## Conditioning a Planner on Another Diffusion Model's Latents (DriveLaW)

[[sources/drivelaw.md]] adds a conditioning source not seen elsewhere on this page: the internal activations of a *second* diffusion model. A 133M vanilla DiT trained with flow matching cross-attends to per-block latents $\{f_1,\dots,f_B\}$ cached from a 2B Video DiT **during its first denoising step**:

$$f_{\theta}(a_{t},t)=\mathrm{DiT}_{\mathrm{act}}\big([\,h_{\mathrm{act}};\,t\,]\,\big|\,h_{\mathrm{ctx}},\{f_{i}\}_{i=1}^{B}\big)$$

Every other diffusion planner tracked here conditions on a perception encoder, a VLM's hidden states, or BEV features. DriveLaW conditions on a generative model mid-generation, and at inference the planner runs entirely in latent space with no video decoding.

**The conditioning timestep is a genuine hyperparameter, and a brittle one.** Latents from denoise step $t{=}1$ give 89.1 PDMS, $t{=}5$ gives 86.9, and $t{=}10$ — nearly clean generated video — collapses the policy to 23.2 PDMS with comfort at 0. A 66-point swing from a conditioning choice is worth flagging for anyone building this architecture: the useful signal is the generator's early internal state, not its finished output, and the failure mode at the clean end is catastrophic rather than gradual. The paper attributes it to redundancy in pixel-format video but offers no deeper diagnosis.

**Planner size is not the bottleneck.** 133M parameters conditioned on good latents reach 89.1 PDMS with no RL and no scorer, above ReCogDrive-IL (86.5), Epona (86.2), and DiffusionDrive (88.1). The representation, not the planner capacity, does the work — consistent with the representation ablation summarized in [[concepts/foundation-backbones-for-ad.md]].
...
Both streams use the same linear-interpolation transport and an **$x$-prediction (clean-endpoint) parameterization**:

$$\mathcal{Z}_{t}=(1-t)\epsilon_{\mathrm{future}}+t\,\mathcal{Z}^{*}_{\mathrm{future}},\qquad 	ilde{\mathcal{Y}}_{t}=(1-t)\epsilon_{y}+t\,ar{\mathcal{Y}}_{H+1:H+K}$$

At each step the predictor estimates clean endpoints, converts them to velocities along the linear paths, and integrates — so the same sampler advances the imagined future and the plan together.

**The coupling is deliberately one-directional.** A stop-gradient sits on the action tokens *in the scene stream only*: the scene output reads actions but its loss never updates the action stream, while the action output reads fully differentiable scene tokens. Action supervision therefore shapes the world representation, but world-modeling cannot perturb the policy. Compare [[sources/drivelaw.md]], where the planner reads a frozen generator's mid-denoising latents with no gradient path at all, and DriveVA, where video and action are denoised jointly with no such asymmetry.

### Flow Matching vs. Regression Is a Measurable Choice

The result most relevant to this page: on the *latent* stream, swapping flow matching for direct regression is not a wash — it is worse than dropping future prediction entirely.

| Stage 2 configuration | EPDMS |
|---|---:|
...
The mechanism is the standard one this page invokes for trajectories — a deterministic L2 objective converges to the conditional mean of a multimodal target — but WA-JEPA is the first ingested paper to *measure* it in latent space rather than argue it. Its two target-referenced diagnostics: the directional-similarity collapse gap falls 0.30 → 0.10 and the change-magnitude ratio rises 0.45 → 0.80 (1.0 is perfect) when moving from regression to flow matching. Regression under-produces temporal variation by more than half.

This corroborates SpanVLA's trajectory-side finding from a different direction: its L1 head loses 5.2 PDMS against an otherwise identical flow-matching action expert (85.1 vs. 90.3). **Two papers, two different prediction targets, same conclusion — deterministic heads pay a real price wherever the target is genuinely multimodal.** The corollary matters too: where the target is *not* very multimodal, deterministic objectives are fine, which is why [[sources/auto-jepa.md]]'s alignment-based prediction of a single ego-trajectory latent works without any generative machinery.

## A Deterministic Counterexample (GeoWAM)

Nearly every entry on this page argues, explicitly or implicitly, that a generative action head beats a deterministic one — SpanVLA's L1 head loses 5.2 PDMS to an otherwise identical flow-matching expert, DiffusionDrive's whole design is about making diffusion cheap enough to deploy, and WA-JEPA extends the argument to latent targets. [[sources/geowam.md]] is a live counterexample and worth keeping on this page for that reason.

Its action head takes historical and predicted-future ego tokens, refines them with a causal temporal transformer, and has a learned trajectory query cross-attend before a regression head emits **one trajectory** — position and heading, $\ell_1$ loss, explicitly "without trajectory anchors, mode classification, or iterative sampling." It reports the top score in its own navtest table (90.2 EPDMS) and leads navhard at 36.6.

**The submetric that explains part of it is extended comfort.** GeoWAM scores EC 86.8 where DVGT-2, EponaV2, and DriveLaW sit at 77.0-77.4, and where sampled planners routinely struggle: [[sources/auto-jepa.md]] 75.2, DriveVLA-W0 58.9, WAM-Flow 73.9. A deterministic head has no sampling noise to produce frame-to-frame jitter, so the metric that punishes inconsistency across paired rollouts costs it nothing. Compare Drive-JEPA, which needed a momentum-aware selector to recover EC from 47.9 to 84.8, and [[sources/wa-jepa.md]], whose HUGSIM closed-loop comfort of 0.662 against ~0.95 for deterministic baselines is the same phenomenon in a reactive setting.

**What it gives up is everything multimodality provides.** No Best-of-N, no proposal diversity, no way to expose alternative maneuvers at an ambiguous intersection, nothing for a scorer or a preference optimizer to rank. Given how much of this wiki's RL and selection literature depends on having a diverse candidate set ([[concepts/best-of-n.md]], [[concepts/intent-conditioned-planning.md]], [[concepts/selection-based-planning.md]]), that is a substantial architectural bet, and GeoWAM does not discuss it.

**The honest reading** is not that deterministic beats generative. GeoWAM's own attribution is +0.6 EPDMS over DVGT-2, which also uses a deterministic head, so the paper contains no evidence about head design at all — it never varies it. What it does establish is that a deterministic regressor with a strong enough conditioning representation is not obviously behind, which the comfort numbers suggest may be a representation-quality result rather than a head-design one. The trade is real: **sampling buys multimodality and costs consistency.**

## Decoupling and Representation Alignment: Buying Denoising Steps With Architecture

[[sources/wcog-vla.md]]'s **ADDT** targets a specific claim about single-network DiTs — that encoding low-frequency abstract semantics conflicts with decoding high-frequency continuous detail — and splits the 16-block model into an 8-block **condition encoder** and an 8-block **generation decoder**. The encoder emits a self-condition feature $ injected into the decoder through AdaLN; both cross-attend to the VLM token sequence.

On top of that it adds **representation alignment**: the 6th encoder block's feature is pulled by cosine similarity toward a latent from a GenAD-style VAE pretrained to reconstruct multi-agent trajectories. The stated purpose is not fidelity but **stability of $ across adjacent denoising timesteps** — an explicitly REPA-flavoured trick applied to a *trajectory* latent space rather than a visual one, which is new here.

The ablation is the useful part, because it measures both mechanisms at two denoising budgets on an identical DiT backbone:

| Variant | 5 steps | 20 steps | Gain from more steps |
| --- | ---: | ---: | ---: |
| SDT (neither) | 87.4 | 88.5 | +1.1 |
| DDT (+ alignment) | 87.9 | 88.7 | +0.8 |
| ADT (+ decoupling) | 88.6 | 89.1 | +0.5 |
| **ADDT (both)** | **89.3** | **89.6** | **+0.3** |

Read down the columns and the two mechanisms are worth **+1.9 PDMS at 5 steps** but only **+1.1 at 20**. Read across the rows and the value of extra denoising *shrinks as the architecture improves* — from +1.1 for the plain DiT to +0.3 for the full design.

**The generalizable statement**: architectural conditioning and denoising budget are substitutes. A planner that stabilizes its conditioning signal does not need to iterate to recover it. That is the mechanism behind several few-step results on this page — [[sources/diffusiondrive.md]]'s 2-step truncated diffusion, [[sources/driveva.md]]'s 2 ODE steps — stated and measured rather than assumed.

It also connects to the world-model side of the wiki: [[sources/adaptive-wam.md]], [[sources/brainwam.md]], and [[sources/drivelaw.md]] all find that iterating a *video* denoiser buys nothing for planning. WCog-VLA finds the same for the *action* denoiser and gives a reason — the conditioning feature is already stable, so further steps have little to correct.
