...
8. **How much perception does a generated future replace?** [[sources/foresight.md]] runs the only direct measurement. Its planner reads a frozen 2.5B generator's imagined future as its primary visual input, with a TransFuser branch supplying multi-view images, LiDAR, and ego status as an explicit "supplement." Deleting that branch entirely costs **1.1 PDMS (89.3 → 88.2)**, concentrated in DAC (97.2 → 96.3) and EP (83.5 → 81.7) — drivable-area compliance and progress, precisely what lateral views and LiDAR geometry inform. TTC actually *improves* (94.8 → 95.4).

    So a front-view generated future substitutes for most, but not all, of a conventional multi-modal perception stack. The residual is a **sensor-coverage** gap rather than a representational one: every foundation world model in this wiki generates a single forward view, while NAVSIM provides 3 cameras and nuScenes 6. ForeSight's own position is that the current encoder becomes unnecessary once multi-view generation matures — untested, and it presumes multi-view generation preserves cross-view geometric consistency well enough to substitute for LiDAR, which no ingested paper has shown.

    Read alongside item 6: Auto-JEPA reaches 91.3 with no perception labels and no spatial output; ForeSight reaches 88.2 with no perception *inputs* beyond a generated front view. Both routes work; neither yields an inspectable scene representation.

9. **Agent tokens as a VLM input, and what they are worth.** [[sources/wcog-vla.md]] takes the most direct route available: a BEVFormer encoder lifts six surround views to BEV, a UniAD-style TrackFormer distils that into sparse agent tokens, and those tokens are concatenated with vision and text tokens straight into an InternVL3-2B LLM. The returned agent hidden states are decoded by a world head into current 3D boxes *and* future agent trajectories, so the same tokens carry perception and prediction.

    Removing the 3D perception module entirely — leaving ADDT to rely on generic VLM vision tokens — costs **3.3 PDMS** (89.3 → 86.0), the largest perception ablation on this page. But the paper's own four-stage table measures 3D perception pretraining at only **+1.1** (84.4 → 85.5) when the planner still outputs text tokens. The two are not reconciled, and the gap is informative: **explicit 3D structure is worth three times more once there is a continuous action head able to use it.** A VLM emitting waypoints as text may simply be unable to exploit precise spatial input.

    Read against item 6 above: Auto-JEPA reaches 91.3 with no perception labels at all, WCog-VLA reaches 92.9 with 3D boxes plus per-agent future trajectories. Both routes work; the supervised one additionally yields an inspectable scene representation, which is the trade this page has been tracking.
