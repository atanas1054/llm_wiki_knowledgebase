...
- **The corroboration**: [[sources/brainwam.md]] reproduces Table 6's conclusion in an unrelated architecture. Its video and action rectified-flow timesteps are independent, so the number of video denoising steps run before the features are cached is a free parameter: **one step gives 89.3 PDMS, two 89.5, three 89.4**, with steps 2-3 costing 169 ms for 0.2. Different backbone (Wan2.2-5B rather than LTX-Video), different coupling (an 8-token action bridge rather than chained latents), same finding — the planner wants the generator's early state, not its finished output. That makes ForeSight the outlier of the three rather than a refutation of this table.
- **The axis, corrected**: [[sources/adaptive-wam.md]] separates the *noise index* from the *DiT readout depth* and finds the first worth <=0.15 PDMS while the second is worth 4.80 — the mid-network block beating the full-depth exit. Since Table 6 reads a fixed layer at varying iteration counts, its t=10 collapse cannot be a pure noise-level effect, which sharpens rather than weakens this page's open question about what actually breaks there (activation statistics after ten iterations is the live hypothesis). It also corroborates the practical conclusion a third time: **one conditional forward is what the planner needs**, which is exactly what t=1 delivers.
- **The dissent**: [[sources/foresight.md]] runs the closest thing to the opposite experiment and gets the opposite answer. Its Table 5 sweeps the *total* denoising budget (25 → 100 steps) and finds PDMS rises monotonically, 88.0 → 89.3, concluding that a more fully formed future plans better. The variables are not identical — DriveLaW moves the extraction point along a fixed schedule, ForeSight changes the schedule length and never reports its extraction step $t_{
m d}$ — so this is an unresolved tension rather than a refutation. It is the first result in the wiki pushing back on Table 6's reading, and it deserves to be weighed before treating "read the generator early, not late" as settled. ForeSight's own answer to the noise problem is architectural: a WM-QFormer that filters the finished future, rather than reading an unfinished one.
